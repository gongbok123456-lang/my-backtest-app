            ...
            with c_lab_in:
                ...
                with st.form("lab_form"):
                    ...
                    with st.expander("🆕 5모드 상세 설정", expanded=False):
                        ...
                        lab_bear_buy = bc1.number_input("매수점%", -1.0, 0.0, min(max(l_mb, -1.0), 0.0), step=0.1, key="bear_buy")
                        lab_bear_prof = bc2.number_input("익절%", 1.5, 3.0, min(max(l_mp, 1.5), 3.0), step=0.1, key="bear_prof")
                        lab_bear_time = bc3.number_input("존버일", 20, 35, min(max(l_mt, 20), 35), step=1, key="bear_time")
                        st.markdown("**🟠 CEILING** (천장 구간)")
                        cc1, cc2, cc3 = st.columns(3)
                        lab_ceil_buy = cc1.number_input("매수점%", -0.5, 0.0, min(max(l_cb, -0.5), 0.0), step=0.1, key="ceil_buy")
                        lab_ceil_prof = cc2.number_input("익절%", 1.0, 2.5, min(max(l_cp, 1.0), 2.5), step=0.1, key="ceil_prof")
                        lab_ceil_time = cc3.number_input("존버일", 30, 50, min(max(l_ct, 30), 50), step=1, key="ceil_time")
                        st.markdown("---")
