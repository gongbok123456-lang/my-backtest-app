...
# --- [UI 구성] ---
# ... (backtest_engine_web 함수 끝나는 부분 이후부터 복사해서 덮어씌우세요) ...
...
# 전역 설정 (공통)
with st.sidebar:
    st.header("⚙️ 기본 설정 (공통)")
    sheet_url = st.text_input("🔗 구글 시트 주소", value=DEFAULT_SHEET_URL)
    
    # 기간 및 자산 설정은 공통으로 사용한다고 가정 (분리 원하시면 탭 안으로 이동 가능)
    st.subheader("💰 자산 및 기간")
    balance = st.number_input("초기 자본 ($) (각 전략별)", value=10000)
    ...
    def render_strategy_inputs(suffix, key_prefix):
        st.subheader(f"📊 {key_prefix} 파라미터")
        profit_rate = st.slider("이익 복리율 (%)", 0, 100, 70, key=f"pr_{suffix}")
        loss_rate = st.slider("손실 복리율 (%)", 0, 100, 50, key=f"lr_{suffix}")
        
        add_order_cnt = st.number_input("추가 주문 횟수", value=4, min_value=1, key=f"add_{suffix}") 
        loc_range = st.number_input("LOC 하단 범위 (-%)", value=20.0, min_value=0.0, key=f"rng_{suffix}")
        ...
        with tab_dash:
            ...
            def render_dashboard(col, p_params, strategy_name):
                with col:
                    ...
                    total_equity = daily_last['Equity']
                    ...
                    # 요약 지표
                    st.metric("시드 자산", f"${seed_equity_basis:,.0f}")
                    st.metric("보유 현금", f"${current_cash:,.0f}")
                    st.caption(f"이격도: {disp:.4f} ({curr_phase})")
                    st.divider()
